...
namespace Org.BouncyCastle.Cms
{
	 ...
	 * </p>
     */
    public class CustomCMSSignedDataGenerator
        : CmsSignedGenerator
    {
        ...
        private class SignerInf
        {
            ...
            internal SignerInfo ToSignerInfo(DerObjectIdentifier contentType, CmsProcessable content, SecureRandom random)
            {
                ...
                ISigner sig = outer._signerProvider != null
                    ? outer._signerProvider : CmsSignedHelper.GetSignatureInstance(signatureName);
                ...
                using (Stream sigStr = new BufferedStream(new SignerSink(sig)))
                {
                    if (sAttr != null)
                    {
        ...
        public CustomCMSSignedDataGenerator(SecureRandom random)
            : base(random)
        {
        }
            ...
            bool encapsulate)
        {
            ...
            foreach (SignerInf signer in signerInfs)
            {
                try
                {
                    ...
                    signerInfos.Add(signer.ToSignerInfo(contentTypeOid, content, m_random));
                }
                catch (IOException e)
                {
        ...
        */
        ...
        public void PreGenerate(string signedContentType, CmsProcessable content)
        {
            ...
            foreach (SignerInformation signer in _signers)
            {
                digestAlgs.Add(CmsSignedHelper.FixDigestAlgID(signer.DigestAlgorithmID, DefaultDigestAlgorithmFinder.Instance));
